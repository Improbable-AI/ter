...
from backtracking.common.running_mean_std import RunningMeanStd
...
class DisCorDQN(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    saved_attributes = ("model", "target_model", "optimizer", "error_model", "error_model_optimizer")
    ...
    def __init__(
        ...
        self.error_model = error_func
        self.error_model_optimizer = error_model_optimizer
        self.error_optim_t = 0
        self.tau = self.tau_init = 10.0
        self.running_mean_std_error = RunningMeanStd()
        ...
        self.delta_record = collections.deque(maxlen=100)
        self.delta_loss_record = collections.deque(maxlen=100)
        self.tau_record = collections.deque(maxlen=100)
    ...
    def sync_target_network(self):
        ...
        if self.target_model is None:
            ...
            if self.error_optim_t < 1:
                self.tau = self.tau_init
            else:
                self.tau = self.running_mean_std_error.mean[0]
                self.running_mean_std_error = RunningMeanStd()
    ...
    def update(self, experiences, errors_out=None):
        ...
        if has_weight:            
            ...
            if errors_out is None:
                errors_out = []
        loss, delta_loss = self._compute_loss(exp_batch, errors_out=errors_out)
        if has_weight:
            if hasattr(self.replay_buffer, 'update_errors'):
                self.replay_buffer.update_errors(errors_out)

        self.loss_record.append(float(loss.detach().cpu().numpy()))
        self.delta_loss_record.append(float(delta_loss.detach().cpu().numpy()))
        ...
        self.error_model_optimizer.zero_grad()
        delta_loss.backward()
        if self.max_grad_norm is not None:
            pfrl.utils.clip_l2_grad_norm_(self.error_model.parameters(), self.max_grad_norm)
        self.error_model_optimizer.step()
        self.error_optim_t += 1
    ...
    def _compute_next_state_error_pred(self, exp_batch):
        with torch.no_grad():
            batch_next_state = exp_batch["next_state"]

            if self.recurrent:
                target_next_error_out, _ = pack_and_forward(
                    self.error_model, batch_next_state, exp_batch["next_recurrent_state"],
                )
            else:
                target_next_error_out = self.error_model(batch_next_state)
            next_error_max = target_next_error_out.max           
        
            return next_error_max

    def _compute_state_error_pred(self, exp_batch):
        batch_size = exp_batch["reward"].shape[0]
        ...
        if self.recurrent:
            error_out, _ = pack_and_forward(
                self.error_model, batch_state, exp_batch["recurrent_state"]
            )
        else:
            error_out = self.error_model(batch_state)

        batch_actions = exp_batch["action"]
        batch_errors = torch.reshape(error_out.evaluate_actions(batch_actions), (batch_size, 1))

        return batch_errors
    
    def _compute_error_target(self, exp_batch, delta, next_error):
        batch_terminal = exp_batch["is_state_terminal"]
        
        return delta + (1.0 - batch_terminal) * self.gamma * next_error
    ...
    def _compute_loss(self, exp_batch, errors_out=None):
        ...
        with torch.no_grad():
            delta = torch.abs(y - t)[:, 0]

        state_error_preds = self._compute_state_error_pred(exp_batch) # \detla(s, a)
        next_state_error_preds = self._compute_next_state_error_pred(exp_batch) # \delta(s', a') -- no grad, no masking
        state_error_targets = self._compute_error_target(exp_batch, delta, next_state_error_preds) # |td error of s,a| + \gamma \delta(s', a') # no grad

        self.q_record.extend(y.detach().cpu().numpy().ravel())
        self.delta_record.extend(next_state_error_preds.detach().cpu().numpy().ravel())
        ...
        if errors_out is not None:
            ...
            if delta.ndim == 2:
                delta_cpu = torch.sum(delta, dim=1)
                
            delta_cpu = delta.detach().cpu().numpy()
            for i, e in enumerate(delta_cpu):
                errors_out.append(e)

        # TODO: compute y weight
        with torch.no_grad():
            # import ipdb; ipdb.set_trace()
            batch_terminal = exp_batch["is_state_terminal"]
            batch_size = exp_batch["reward"].shape[0]
            delta_weights = -(1.0 - batch_terminal) * self.gamma *  next_state_error_preds / self.tau # add an epsilon to prevent zero weight
            # normalized_weights = (delta_weights / delta_weights.sum())
            normalized_weights = batch_size * torch.nn.functional.softmax(delta_weights, dim=0)
            self.tau_record.append(self.tau)
            # self.tau = (1 - 1.0 / self.target_update_interval) *  self.tau  +  (1.0 / self.target_update_interval) * next_state_error_preds.mean().cpu().numpy()
            self.running_mean_std_error.update(state_error_preds.detach().cpu().numpy())

        if "weights" in exp_batch:
            qloss = compute_weighted_value_loss(
                y,
                t,
                normalized_weights * exp_batch["weights"],
                clip_delta=self.clip_delta,
                batch_accumulator=self.batch_accumulator,
            )
            delta_loss = compute_value_loss(
                state_error_preds,
                state_error_targets ,
                clip_delta=self.clip_delta,
                batch_accumulator=self.batch_accumulator,
    ...
    def get_statistics(self):
        agent_statistics = [
            ...
            ("average_delta", _mean_or_nan(self.delta_record)),
            ("average_delta_loss", _mean_or_nan(self.delta_loss_record)),
            ("tau", self.tau),
        ...
        if hasattr(self.replay_buffer, 'get_statistics'):
            agent_statistics += self.replay_buffer.get_statistics()
